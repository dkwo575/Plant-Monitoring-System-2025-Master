...
def main(config_file, checkpoint_file, image_path, depth_path, output_path):
    ...
    # Define custom colors for each class label
    colors = [
        (250, 10, 10),  # Red color for "ripened" label
        (255, 165, 0),  # Orange color for "half_ripened" label
        (0, 163, 24)  # Green color for "green" label
    ]

    # Separate bbox and segmentation results
    bbox_result, segm_result = result if isinstance(result, tuple) else (result, None)

    # Initialize an empty list to hold label indices
    labels = [np.full(bbox.shape[0], i, dtype=np.int32) for i, bbox in enumerate(bbox_result)]
    labels = np.concatenate(labels)
    bboxes = np.vstack(bbox_result)
    ...
    # Save the output image with custom colors
    model.show_result(img, result, out_file=output_path, mask_color=colors)
...
if __name__ == '__main__':
    ...
    output_path = 'E:/Code/Python_WorkSPace/AI_Combine_Realsense/mmdetection/client_ai/20240530_114523_color_processed.png'
