...
// import { Card, Button, Badge, Space, Typography, Statistic, message } from 'antd';
// import { CameraOutlined } from '@ant-design/icons';
// import type { CameraFeedProps, CapturedImage } from '../types';
...
import React, { useState, useEffect, useRef } from 'react';
import { Card, Button, Badge, Space, Statistic, message } from 'antd';
import { CameraOutlined } from '@ant-design/icons';
...
const CameraFeed: React.FC<CameraFeedProps> = ({
  ...
  const captureImage = async () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        // Mocking the fetch call since the endpoint is local
        // const response = await fetch('http://localhost:5000/upload', {
        //   method: 'POST',
        //   body: formData,
        // });
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.code === 'Space' && isConnected && !isCapturing) {
        e.preventDefault();
        captureImage();
      }
    };
    ...
    window.addEventListener('keydown', handleKeyPress);
    return () => window.removeEventListener('keydown', handleKeyPress);
  }, [isConnected, isCapturing]); // Added isCapturing to dependencies
