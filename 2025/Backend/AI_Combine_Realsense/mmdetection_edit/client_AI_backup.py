...
def attempt_processing(filename):
    ...
    if image_file in pending_images and depth_file in pending_depths:
        print(f"Processing image '{image_file}' with depth data '{depth_file}'")
        processed_file_path = process_files(image_file, depth_file)
        if processed_file_path:
            upload_file(upload_url, processed_file_path)
        # Remove from pending lists after processing
        pending_images.remove(image_file)
        pending_depths.remove(depth_file)
    else:
